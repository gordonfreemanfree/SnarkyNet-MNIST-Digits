...
describe('int', () => {
  ...
  describe('Int65', () => {
    ...
    describe('zero', () => {
      it('should be the same as Field(0)', async () => {
        expect(Int65.zero.magnitude.value).toEqual(Field(0));
      });
    });

    describe('fromUnsigned', () => {
      it('should be the same as UInt64.zero', async () => {
        expect(new Int65(UInt64.zero, Sign.one)).toEqual(
          Int65.fromUnsigned(UInt64.zero)
        );
      });

      it('should be the same as UInt64.MAXINT', async () => {
        expect(Int65.from((1n << 64n) - 1n)).toEqual(
          Int65.fromUnsigned(UInt64.MAXINT())
        );
      });
    });
    ...
    describe('neg', () => {
      ...
      it('neg(2^53-1)=-2^53-1', () => {
        const int = Int65.from(NUMBERMAX);
    ...
    describe('add', () => {
      it('1+1=2', () => {
        expect(Int65.one.add(Int65.from('1')).toString()).toEqual('2');
      });

      it('5000+(-4000)=1000', () => {
        expect(
          Int65.from(5000)
            .add(Int65.fromField(Field(-4000)))
            .toString()
        ).toEqual('1000');
      });
      ...
      it('(MAXINT/2+MAXINT/2) adds to MAXINT', () => {
        ...
        expect(
          Int65.from(value).add(Int65.from(value)).add(Int65.one).toString()
      ...
      it('should throw on overflow', () => {
        ...
        expect(() => {
          Int65.from(100).add(1n << 64n);
        }).toThrow();
        expect(() => {
          Int65.from(100).sub('1180591620717411303424');
        }).toThrow();
        expect(() => {
          Int65.from(100).mul(UInt64.from(Field(1n << 100n)));
        }).toThrow();
      });

      // TODO - should we make these throw?
      // These are edge cases, where one of two inputs is out of the Int65 range,
      // but the result of an operation with a proper Int65 moves it into the range.
      // They would only get caught if we'd also check the range in the Int65 / UInt64 constructors,
      ...
      it.skip('operations should throw on overflow of any input', () => {
        expect(() => {
          new Int65(new UInt64(Field(1n << 64n))).sub(1);
        }).toThrow();
        expect(() => {
          new Int65(new UInt64(Field(-(1n << 64n)))).add(5);
        }).toThrow();
        expect(() => {
          Int65.from(20).sub(new UInt64(Field((1n << 64n) + 10n)));
        }).toThrow();
        expect(() => {
          Int65.from(6).add(new UInt64(Field(-(1n << 64n) - 5n)));
        }).toThrow();
      });

      it('should throw on overflow addition', () => {
        expect(() => {
          Int65.from((1n << 64n) - 1n).add(1);
        }).toThrow();
        expect(() => {
          Int65.one.add((1n << 64n) - 1n);
        }).toThrow();
      });
      it('should not throw on non-overflowing addition', () => {
        expect(() => {
          Int65.from((1n << 64n) - 1n).add(Int65.zero);
        }).not.toThrow();
      });
    });
    ...
    describe('sub', () => {
      it('1-1=0', () => {
        expect(Int65.one.sub(Int65.from(1)).toString()).toEqual('0');
      });

      it('10000-5000=5000', () => {
        expect(
          Int65.fromField(Field(10000)).sub(Int65.from('5000')).toString()
        ).toEqual('5000');
      });

      it('0-1=-1', () => {
        expect(Int65.zero.sub(Int65.one).toString()).toEqual('-1');
      });

      it('(0-MAXINT) subs to -MAXINT', () => {
        expect(Int65.zero.sub(UInt64.MAXINT()).toString()).toEqual(
    ...
    describe('toFields', () => {
      it('toFields(1) should be the same as [Field(1), Field(1)]', () => {
        expect(Int65.toFields(Int65.one)).toEqual([Field(1), Field(1)]);
      });

      it('toFields(2^53-1) should be the same as Field(2^53-1)', () => {
        expect(Int65.toFields(Int65.from(NUMBERMAX))).toEqual([
    ...
    describe('fromFields', () => {
      it('fromFields([1, 1]) should be the same as Int64.one', () => {
        expect(Int65.fromFields([Field(1), Field(1)])).toEqual(Int65.one);
      });

      it('fromFields(2^53-1) should be the same as Field(2^53-1)', () => {
        expect(Int65.fromFields([Field(String(NUMBERMAX)), Field(1)])).toEqual(
          Int65.from(NUMBERMAX)
        );
      });
    });
